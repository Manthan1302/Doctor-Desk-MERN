...
function DoctorAppointments({ updateDoctor, updateDoctorToken }) {
  ...
  const getPendingAppointments = async () => {
    ...
    try {
      ...
      const response = await axios.get("http://localhost:8888/doctor/getAllAppointment", headers);
  ...
  const updateAppointment = async (item) => {
    ...
    try {
      const headers = { headers: { Authorization: `Bearer ${token}` } };
      console.log(headers);
      const response = await axios.put(`http://localhost:8888/doctor/acceptAppointment/${_id}`, headers);
      console.log(response);
      if (response) {
          ...
          <div class="container">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
              <ul class="navbar-nav mx-auto">
                ...
                <li class="nav-item">
                  <a class="nav-link" href="/doctor" onClick={() => { updateDoctor({}); updateDoctorToken({}) }} >Logout</a>
                </li>
            ...
            <tbody>
              {
                pendingAppointment.map((item) => {
                  if (item.TimeSlot === "9-12" && item.appointmentStatus === "Pending") {
                    return (
                      <tr>
                        ...
                        <td>
                          <button type="submit" class="btn btn-success" onClick={() => updateAppointment(item)}>Accept</button>
                        </td>
                        <td>
                          <button type="submit" class="btn btn-danger" onClick={() => rejectAppointment(item)}>Reject</button>
                        </td>
                      </tr>
                    )
                  }
        ...
        <section style={{ padding: "15px" }}>
          ...
          <table class="table">
            ...
            <tbody>
              {
                pendingAppointment.map((item) => {
                  if (item.TimeSlot === "4-7" && item.appointmentStatus === "Pending") {
                    return (
                      <tr>
                        <td>{item.patient.patientName}</td>
                        <td>{item.patient.patientPhoneNumber}</td>
